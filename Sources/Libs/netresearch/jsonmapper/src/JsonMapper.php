 ...
 *
 * @package Ppfeufer_Theme_EVEOnline_JsonMapper
 * @author  Christian Weiske <cweiske@cweiske.de>
 * @license OSL-3.0 http://opensource.org/licenses/osl-3.0
 * @link    http://cweiske.de/
 */
class Ppfeufer_Theme_EVEOnline_JsonMapper
{
    /**
    ...
    /**
     * Override class names that Ppfeufer_Theme_EVEOnline_JsonMapper uses to create objects.
    ...
    public function map($json, $object)
    {
        if ($this->bEnforceMapType && !is_object($json)) {
            throw new InvalidArgumentException(
                'Ppfeufer_Theme_EVEOnline_JsonMapper::map() requires first argument to be an object'
                . ', ' . gettype($json) . ' given.'
            );
        }
        if (!is_object($object) && (!is_string($object) || !class_exists($object))) {
            throw new InvalidArgumentException(
                'Ppfeufer_Theme_EVEOnline_JsonMapper::map() requires second argument to '
        ...
        foreach ($json as $key => $jvalue) {
            ...
            if (!$hasProperty) {
                if ($this->bExceptionOnUndefinedProperty) {
                    throw new Ppfeufer_Theme_EVEOnline_JsonMapper_Exception(
    ...
    protected function inspectProperty(ReflectionClass $rc, $name)
    {
        ...
        //we have to look it up in the class Ppfeufer_Theme_EVEOnline_hierarchy
        $class = $rc;
        $rprop = null;
        do {
