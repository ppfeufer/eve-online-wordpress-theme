...
jQuery(function($) {
    ...
    function fixAutomaticMenu(device) {
        let $autMenuItems = $('ul.nav > li.page_item_has_children');
     ...
     */
    const $oEmbedVideos = $('iframe[src*="youtube"], iframe[src*="vimeo"], iframe[src*="videopress"]');
    $oEmbedVideos.each(function() {
        // adding the youtube video ID to the iframe
        const youtubeVideoData = $(this).attr('src').match(/^.*(?:(?:youtu\.be\/|v\/|vi\/|u\/\w\/|embed\/)|(?:(?:watch)?\?vi?=|&vi?=))([^#&?]*).*/);

        if(youtubeVideoData !== null) {
            const youtubeVideoID = youtubeVideoData['1'];
     ...
     */
    const $externalLinks = $('a[rel="external"]');
